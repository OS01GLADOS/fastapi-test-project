...
async def get(id: int, session: AsyncSession):
    ...
    if not res:
        return None
    query_res = await session.execute(select(models.Blog).where(models.Blog.user_id==id))
    blogs = query_res.all()
    res = dict(res)
    res['blogs']= blogs
    return res
...
async def create(request: User,session: AsyncSession):
    ...
    query_res = await session.execute(select(models.User).where(models.User.email == request.email))
