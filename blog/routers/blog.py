...
async def create(
    request: Blog, get_current_user: User = Depends(get_current_user)
):
    async with engine.connect() as conn:
...
async def show(id: int, get_current_user: User = Depends(get_current_user)):
    async with engine.connect() as conn:
...
async def update(
    id: int, request: Blog, get_current_user: User = Depends(get_current_user)
):
    async with engine.connect() as conn:
        user_mail = get_current_user.email
        blog = await blog_repo.change(user_mail, id, request, conn)
        if not blog:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f'Blog with the id {id} is not available',
...
async def destroy(id: int, get_current_user: User = Depends(get_current_user)):
    async with engine.connect() as conn:
        user_mail = get_current_user.email
        res = await blog_repo.destroy(user_mail, id, conn)
        if not res:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f'Blog with the id {id} is not available',
